# KIT INSTITUCIONAL DE ENTRADA: EMENTA OFICIAL, GUIA TÉCNICO E TESTE DE NIVELAMENTO
...
* **Propriedade fundamental:** Retornos simples se somam **entre ativos no espaço (cross-section)**. Se uma carteira aloca pesos $w_i$, o retorno total do portfólio no dia é exatamente a média ponderada:
...
* **Propriedade fundamental:** Retornos logarítmicos se somam **ao longo do tempo (série temporal)**. O retorno composto de um período de $T$ dias é simplesmente a soma direta dos log-retornos diários:
...
1. **Sharpe Ratio:**
   ...
   Onde $R_f$ é a taxa livre de risco anualizada (geralmente o CDI no Brasil).
...
* **Questão 1: B**  
  *Comentário:* A agregação transversal (cross-section) de múltiplos ativos obedece estritamente à linearidade dos retornos simples: $R_p = \sum w_i R_i$. Somar log-retornos ponderados de ativos gera distorções matemáticas graves na carteira.
* **Questão 2: B**  
